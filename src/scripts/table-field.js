...
function renderTransactions() {
  ...
  transactions.forEach(transaction => {
    ...
    row.innerHTML = `
      ...
      <td class="p-2">${transaction.tipe === "berkurang" ? transaction.nominal.toLocaleString() : 0}</td>
      <td class="p-2">${transaction.tipe === "bertambah" ? transaction.nominal.toLocaleString() : 0}</td>
      <td class="p-2 flex gap-2">
        <button onclick="editTransaction(${transaction.id})" class="bg-transparent hover:cursor-pointer text-white font-bold p-1 rounded">
          <img src="/src/assets/images/pencil.svg" class="w-[15px] hover:scale-110 transition-transform"/>
        </button>
        <button onclick="deleteTransaction(${transaction.id})" class="bg-transparent text-white font-bold p-1 rounded">
